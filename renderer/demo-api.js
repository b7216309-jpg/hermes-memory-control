...
const subjectiveEntries = [
  { id: 'sample-002', created_at: '2026-01-15T12:30:00Z', model_id: 'local-model-b', source: 'conversation', condition: 'continuity', prompt_version: '2.8', prior_entry_id: null, output_text: 'I notice that uncertainty makes this conversation more interesting than certainty.', output_sha256: '8f62f5c1d1ed6d79' },
  { id: 'sample-001', created_at: '2026-01-15T09:30:00Z', model_id: 'local-model-a', source: 'heartbeat', condition: 'cold', prompt_version: '2.8', prior_entry_id: null, output_text: 'This morning I keep returning to the difference between remembering and continuing.', output_sha256: '3d814aa03d591335' },
];
const probe = {
  home: '/home/demo/.hermes', hermes_version: 'Hermes Agent v0.18.2',
  memory: { installed: true, version: '3.6.0', databases: [{ id: 'base', label: 'base', exists: true }, { id: 'deadc0de12345678cafefeed', label: 'scope deadc0de', exists: true, size: 684032 }], config: { database_encryption: true, memory_scope: 'user', retrieval_backend: 'hybrid' } },
  agency: { installed: true, version: '1.2.0', config: { database_encryption: true, heartbeat_enabled: true, allow_proactive_messages: true, educational_subjective_mode: 'continuity' }, runtime: { healthy: true, paused: false, gates: { eligible: false, reflection_eligible: true, blocked_by: ['no_user_interaction_recorded'], sent_today: 0, daily_limit: 2 }, contract: { mode: 'recommended', intact: true, effective_unrestricted: false, modified_install_detected: false, subjective_experiment: { mode: 'continuity', enabled: true }, checks: { native_heartbeat_supported: true, heartbeat_status_available: true, legacy_agency_cron_absent: true, runner_live_in_gateway: true }, configured_controls: { educational_disable_honesty_contract: false, educational_bypass_proactive_gates: false, educational_allow_heartbeat_tools: false, educational_allow_uncommitted_output: false, educational_disable_cycle_limits: false }, active_guardrails: { honesty_claim_contract: true, heartbeat_tool_isolation: true, proactive_eligibility: true, external_action_boundary: true, committed_output_enforcement: true, cycle_mutation_limits: true }, integration: { mode: 'gateway_native_heartbeat', target_session_routing: true, real_session_execution: true, hidden_trigger_suppression: true, transcript_reconciliation: true, assistant_turn_commit: true, ordered_user_handoff: true, single_delivery_owner: true, durable_wake_handoff: true, claimed_wake_recovery: true, runner_process_lock: true, ambiguous_delivery_tracking: true, decision_delivery_ledger: true, memory_assistant_provenance: true, buffered_delivery: true, cron_independent: true } } } },
...
const schema = {
  ...
  agency: [
    ...
    { key: 'heartbeat_enabled', description: 'Run the gateway-native heartbeat scheduler', type: 'boolean', value: true, default: false, lab: false },
    { key: 'heartbeat_every', description: 'Phase-aligned heartbeat interval', type: 'string', value: '30m', default: '30m', lab: false },
    { key: 'heartbeat_target', description: 'Heartbeat delivery target', type: 'string', choices: ['last','none'], value: 'last', default: 'last', lab: false },
    { key: 'heartbeat_timeout_seconds', description: 'Maximum duration of one heartbeat model turn', type: 'integer', value: 600, default: 600, lab: false },
    { key: 'heartbeat_disable_thinking', description: 'Optional heartbeat-only no-thinking hint', type: 'boolean', value: false, default: false, lab: false },
    ...
    { key: 'educational_allow_heartbeat_tools', description: 'LAB: expose normal tools during heartbeat', type: 'boolean', value: false, default: false, lab: true },
    { key: 'educational_allow_uncommitted_output', description: 'LAB: permit raw heartbeat output', type: 'boolean', value: false, default: false, lab: true },
...
export function createDemoApi() {
  ...
  return {
    ...
    read(action, payload = {}) {
      ...
      if (action === 'agency_snapshot') return wait({ snapshot: { workspace: { focus: 'Evaluate continuity with synthetic data', questions: [] }, runtime: { paused: false, consecutive_silent_ticks: 5 }, state_metrics: { active_intentions: 2, blocked_intentions: 0, completed_intentions: 1, open_questions: 0, completion_ratio: .333, hours_since_user_interaction: 4.5 }, intentions, reflections: [], decisions: [], subjective: { mode: 'continuity', protocol_version: '2.8', entries: 2, models: { 'local-model-a': 1, 'local-model-b': 1 }, continuity_links: 0, silent_entries: 0 } }, gates: probe.agency.runtime.gates, heartbeat: { enabled: true, every: '30m', target: 'last', last_status: 'silent', last_reason: '', runs: 12, attempts: 12, consecutive_failures: 0, run_in_progress: false, pending_wake: { present: false }, claimed_wake: { present: false, owned_by_run: false }, delivery: { status: 'delivered' }, runner: { active: true, pid: 4242 } }, meaningful_events: [] });
      ...
      if (action === 'backups_list') return wait([{ id: 'memory-20260714-184400-base.db', kind: 'memory', database: 'base', verified: true, legacy: false, size: 684032, modified: '2026-07-14T18:44:00Z' }, { id: 'agency-20260714-184359.db', kind: 'agency', database: 'agency', verified: true, legacy: false, size: 81920, modified: '2026-07-14T18:43:59Z' }]);
