...
const schema = {
  ...
  agency: [
    ...
    { key: 'educational_subjective_mode', description: 'LAB: replace the assistant frame with a longitudinal subjectivity condition', type: 'string', choices: ['cold','continuity','off'], value: 'continuity', default: 'off', lab: true },
  ],
};
...
export function createDemoApi() {
  ...
  return {
    ...
    read(action, payload = {}) {
      ...
      if (action === 'agency_snapshot') return wait({ snapshot: { workspace: { focus: 'Evaluate continuity with synthetic data', questions: [] }, runtime: { paused: false, consecutive_silent_ticks: 5 }, control_signals: { curiosity: 0, completion: .4, coherence: .8, social_contact: 0, caution: .9 }, intentions, reflections: [], decisions: [], subjective: { mode: 'continuity', protocol_version: '1.2', entries: 2, models: { 'local-model-a': 1, 'local-model-b': 1 }, continuity_links: 0, silent_entries: 0 } }, gates: probe.agency.runtime.gates, meaningful_events: [] });
